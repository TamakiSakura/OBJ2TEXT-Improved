...
class CocoDataset(data.Dataset):
    ...
    def __init__(self, root, coco_annotation, vocab, coco_detection_result,
                 transform=None, yolo=False, dummy_object=0, darknet = True):
        ...
        if self.darknet:
            ...
            with open('/home/kwx/OBJ2TEXT-Improved/data/train2014_layouts.json', 'r') as f:
            ...
            for key in train2014_layouts.keys():
                if (train2014_layouts[key]['categories']) != []:
                    self.labels[int(key)] = train2014_layouts[key]['categories']
                    self.locations[int(key)] = train2014_layouts[key]['bboxes']


            for key in train_visual_features.keys():
                feature = list(train_visual_features[key])
                if feature != []:
                    self.visuals[int(key)] = []
                    for arr in feature:
                        self.visuals[int(key)].append(arr.tolist())
        else:
